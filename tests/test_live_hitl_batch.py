...
def run_test():
    ...
    proposals = [{
        ...
        "strategy": {"type": "mock_strategy"},
        "next_temperature": "warm",
        "next_cool_step": 0
    }]
    
    summary = f"⚙️ E2E Test Warmup proposal for {len(proposals)} seeker:\n\n1. {test_thread_name}: {mock_message_text}\n"
    ...
    if args.simulate:
        ...
        conn.execute(
            ...
            (msg_id, "warmup", json.dumps({"proposals": proposals, "page_id": page_id}), "pending", summary)
        )
        conn.commit()
        conn.close()
    else:
        msg_id = send_proposal_to_telegram("warmup", "batch", summary, {"proposals": proposals, "page_id": page_id})
