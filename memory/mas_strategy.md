...
```text
         ...
         ▼                        ▼                       ▼
  ┌──────────────┐      ┌──────────────┐       ┌──────────────────┐
  │ INBOX MAS    │      │ ROUTE 1:     │       │ ROUTE 2:         │
  │ (Reactive)   │      │ REACT        │       │ WARM-UP          │
  │              │      │              │       │ (Proactive)      │
  │ • Classify   │      │ • ❤️ React   │       │                  │
  │ • Auto-reply │      │ • Reply      │       │ • Nurture        │
  │ • Telegram   │      │   comment    │       │ • Re-engage      │
  │   notify     │      │              │       │ • Remind         │
  └──────────────┘      └──────────────┘       └──────────────────┘
         │                                              │
         └──────────────────┬───────────────────────────┘
...
| Kênh | Hành động | MAS Route |
| --- | --- | --- |
| **Auto-reply** | Trả lời ngay khi seeker hỏi → cung cấp thông tin chương trình từ `lop-hoc.md` / `su-kien.md` | Inbox MAS |
...
- **Hành động**: Phản hồi nhanh (< 2h), cung cấp giá trị liên tục
- **MAS**: Inbox MAS auto-reply + Route 1 React ngay lập tức
...
| --- | --- | --- |
| **Inbox MAS** | Auto-classify và reply tin nhắn | All stages (reactive) |
...
```text
  ...
  │  3. Telegram     │──→ notify yogis khi cần follow-up thủ công
  └──────────────────┘
  ...
  PHASE 5: MESSAGE QA GATE (Kiểm duyệt tin nhắn)
  ───────────────────────────────────────────────
      │
      ▼
  ┌─────────────────────────────────────────────────┐
  │  CHECK 1: last_warmup_at > 7 days?  ──→ ⛔/✅  │
  │  CHECK 2: temperature ≠ unsub/dorm? ──→ ⛔/✅  │
  │  CHECK 3: personalized (tên/city)?  ──→ ⚠️/✅  │
  │  CHECK 4: tone ấm áp, không sales?  ──→ ⚠️/✅  │
  │                                                 │
  │  ALL ✅ ──→ GỬI TIN NHẮN                        │
  │  ANY ⛔ ──→ BLOCK                                │
  │  ANY ⚠️ ──→ REVIEW thủ công                      │
  └─────────────────────────────────────────────────┘


  PHASE 6: EXECUTION (Thực thi)
  ─────────────────────────────
      │
      ▼
  ┌─────────────────────────────────────────────────────┐
  │  Route 1: React    ──→ ❤️ react + reply comment     │
  │  Route 2: Warm-up  ──→ DM proactive message         │
  │  Route 3: Event    ──→ City-targeted event notify    │
  │  Inbox MAS         ──→ Auto-reply DM                 │
  │                                                      │
