...
2. **ADK Commands**: Run locally with `adk run adk_agents/` (CLI) or `adk web adk_agents/` (Web UI).
3. **Agent Naming**: Use descriptive names matching their organization role (e.g., `Receptionist`, `Guide`, `Supervisor`).
...
6. **State Keys**: Use consistent state keys across agents: `classification`, `seeker_context`, `reply_text`, `thread_messages`.
