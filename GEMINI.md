...
## 11. Standalone Server Deployment (10.0.1.42)

- Production runs on `10.0.1.42` from `/mnt/s2/sahajayogavn/funnel-tracking` (user `steve`, no root). Full guide: `deploy/server/README.md` (`doc:server-deploy-001`).
- `deploy/server/setup.sh` is the idempotent bootstrap: Python 3.13 `.venv` via `uv` from `deploy/server/requirements.lock.txt`, user-level Google Chrome in `~/.local/opt/google-chrome`, production web build, and `systemctl --user` units rendered from `deploy/server/systemd/`.
- Units: `funnel-web` (:9995), `funnel-cdp-browser` (CDP :9222, `tools/start_cdp_browser.sh run`), `funnel-inbox-loop`, `funnel-hitl-execution` (draft-only), `funnel-scheduler`. Service logs go to `logs/services/`.
- PostgreSQL is the `postgres-funnel-tracking` container on the same host, bound only to `10.0.1.42:5432`, so `DATABASE_URL` keeps that address even on the server.
- Chrome cookies do not survive a macOS → Linux move (Keychain encryption): after a migration the Facebook session must be re-established once via the server desktop (RustDesk).
